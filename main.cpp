...
#define NECK_RZ_THRESHOLD 30
#define NECK_RZ_ACCELERATION 0.4
...
void driveNeckRZ(void) {
    int target_count = (neck_rz_sign - 0x80) * 2;
    double current_count = RE_neckrz.Get_Count();
    double current_speed = M_neckrz.read();
    double target_speed;
    
    if(target_count - NECK_RZ_THRESHOLD > current_count) target_speed = 1.0;
    else if(target_count + NECK_RZ_THRESHOLD < current_count) target_speed = -1.0;
    else target_speed = 0;
